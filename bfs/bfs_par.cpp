...
void walkNeighbourNodes(int v, VertexBag *writeBag, int *level, int *parent,
                        int thislevel, graph *G) {
    int w, e;
...
VertexBag *
splitAndMergeBag(VertexBag *inbag, int start, int end, int *level, int *parent,
                 int thislevel, graph *G) {
    if ((end - start) > 128) {
        ...
        int mid = (end + start) / 2;
        leftbag = cilk_spawn
        splitAndMergeBag(inbag, start, mid, level, parent, thislevel, G);
        rightbag = splitAndMergeBag(inbag, mid + 1, end, level, parent,
                                    thislevel, G);
        cilk_sync;
...
void bfs(int s, graph *G, int **levelp, int *nlevelsp, int **levelsizep,
         int **parentp, double *nedgest) {
    ...
    while (!readBag->isEmpty()) {
        ...
        bag = cilk_spawn
        splitAndMergeBag(readBag, 0, readBag->size() - 1, level, parent,
                         thislevel, G);
        cilk_sync;
